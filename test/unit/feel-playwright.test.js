...
const planted = { plantedAt: 1000, plantedMoves: [{ dx: 0, dy: 48 }] };
...
// a row-laid body (sidebar plus main) takes the row as a column, so its siblings move sideways only
test('plantedShift credits a row that pushed the page sideways', () => {
  const sideways = { plantedAt: 1000, plantedMoves: [{ dx: 79.1, dy: 0 }] };
  assert.equal(plantedShift(shiftAt(1012, 0, 79.1), sideways), true);
  assert.equal(plantedShift(shiftAt(1012, 0, 78.6), sideways), true, 'a rect snapped to the device pixel still matches');
  assert.equal(plantedShift(shiftAt(1012, 0, 3), sideways), false, 'a sideways move the row never made');
  assert.equal(plantedShift(shiftAt(1012, 79.1, 0), sideways), false, 'the right distance on the wrong axis');
});
...
test("plantedShift never credits the page's own shift", () => {
  ...
  assert.equal(plantedShift(shiftAt(1012, 48, 3), planted), false, 'the right drop, dragged sideways by a move the row never made');
  assert.equal(plantedShift(shiftAt(1012, 48), { plantedAt: 1000, plantedMoves: [] }), false, 'a row that moved nothing');
  assert.equal(plantedShift(shiftAt(1012, 48), { plantedAt: null, plantedMoves: [{ dx: 0, dy: 48 }] }), false, 'a row that never landed');
