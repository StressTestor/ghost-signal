...
10. the frame check's blind lead, closed in the plan task 4 review. 7.7 used to open no interval on the start mark, so a task between the start mark and the window's first `BeginMainThreadFrame` landed in no frame, and a 20 to 49ms burn there passed both the frame and the task check (on the recorded fixtures the composite window lost 5.59 of its 16.8ms of task cpu this way, and every window of a recorded gallery run had some lead cpu, up to 6.8ms). `page.keyboard` and `page.evaluate` steps were the exposed ones, since `locator.click`'s stability wait usually puts a frame ahead of the input. the frame row now opens the first interval on the start mark, so the lead is a frame like any other.
